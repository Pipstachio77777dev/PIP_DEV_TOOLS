...
class JsonEditorApp:
    def __init__(self, root):
        ...
        btn_agregar_campo = tk.Button(campos_frame, text="Agregar campo a todos", command=self.agregar_campo_global)
        btn_agregar_campo.pack(side=tk.LEFT, padx=5)
    ...
    def agregar_campo_global(self):
        nombre = simpledialog.askstring("Agregar campo", "Nombre del nuevo campo:")
        if not nombre:
            return

        if nombre in self.campos:
            messagebox.showwarning("Campo existente", f"El campo '{nombre}' ya existe.")
            return

        valor = simpledialog.askstring("Valor del campo", f"Valor para el campo '{nombre}':")
        if valor is None:
            return
        ...
        messagebox.showinfo("Campo agregado", f"Campo '{nombre}' agregado a todos los elementos.")
        self.actualizar_campos()
        self.refrescar_lista()
